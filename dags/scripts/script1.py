...
def transform_data():
    df_helper = pd.DataFrame(create_df())
    titles = pd.DataFrame({
        'Team':
            ['Sevilla','Espanyol','Villarreal','Rayo Vallecano','Real Sociedad','Girona','Real Madrid','Mallorca',
                 'Atletico Madrid','Elche','Real Valladolid','Cadiz','Real Betis','Valencia','Barcelona','Almeria',
                 'Getafe','Celta Vigo','Osasuna','Athletic Club'],
        'Short Title of Team':
            ['SEV','ESP','VIL','RVL','SOC','GIR','RMA','MAL','ATL','ELC','VAL','CAD','BET','VAL','BAR','ALM','GET',
             'CEL','OSA','ATH']
    })
